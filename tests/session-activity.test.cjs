...
test("running and waiting sessions precede selected offline sessions, and expire independently", () => {
  ...
  try {
    ...
    const api = vm.runInNewContext(storage + "\n({listSessions, getSessionActivity});", { require });
    const now = Date.now();
    ...
    session("working", now - 100000, "Assigned task");
    ...
    assert.equal(api.getSessionActivity("working", now + 600000).activity, "offline");
    assert.equal(api.getSessionActivity("stopped").activity, "offline");
    assert.equal(api.getSessionActivity("stale_task").activity, "offline");
